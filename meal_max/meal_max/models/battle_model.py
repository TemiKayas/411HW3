...
class BattleModel:
    """A model that manages battles between combatants in a meal-based competition."""
    ...
    def battle(self) -> str:
        ...
        Returns:
            str: The name of the winning combatant's meal.
        """
    ...
    def get_battle_score(self, combatant: Meal) -> float:
        ...
        Calculates the battle score for a combatant based on the following rules:
            - Multiply the price by the number of letters in the cuisine.
            - Subtract a difficulty modifier (HIGH = 1, MED = 2, LOW = 3).

        Args:
            combatant (Meal): A meal dataclass representing the combantant.
    ...
    def prep_combatant(self, combatant_data: Meal):
        """Adds a combatant to the combatants list if there is space.

        Args:
            combatant_data (Meal): The combatant data to be added.
